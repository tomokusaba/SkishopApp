...
public record UserDeletedEvent(string UserId, string CorrelationId);

public class UserDeletedEventConsumer(
    IOptions<KafkaSettings> kafkaOptions,
    IServiceScopeFactory scopeFactory,
    ILogger<UserDeletedEventConsumer> logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        var config = new ConsumerConfig
        {
            ...
            GroupId = $"{settings.GroupId}-user-deleted",
            AutoOffsetReset = AutoOffsetReset.Earliest,
            EnableAutoCommit = false
        };
        _consumer = new ConsumerBuilder<string, string>(config).Build();
        _consumer.Subscribe("user.deleted");
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                var @event = JsonSerializer.Deserialize<UserDeletedEvent>(
                ...
                if (@event is not null)
                {
                    ...
                    await pointService.AnonymizeUserDataAsync(
                        @event.UserId, stoppingToken);
                    logger.LogInformation(
                        "ユーザー削除: データ匿名化完了 UserId={UserId}",
                        @event.UserId);
                }
