...
### マイグレーションファイルの管理
- EF Core Migrations を使用する（`dotnet ef migrations add <Name>`, `dotnet ef database update`）
- 手動 SQL マイグレーションが必要な場合は、ファイル名にバージョン番号と説明を含める（例: `V1__create_users.sql`, `V2__add_email_index.sql`）
- **既に適用済みのマイグレーションファイルを修正しない**（新しいマイグレーションで対応する）
- マイグレーションの依存関係を考慮した順序にする
- EF Core の `Migrations/` ディレクトリに自動生成されたファイルは必ずコミット対象とする
